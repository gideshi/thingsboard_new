...
//            device.setTypeId(device.getTypeId());
...
//    }
    ...
    @RequestMapping(value = "/tenant/devices", params = {"limit"}, method = RequestMethod.GET)
    @ResponseBody
    public TextPageData<DeviceInfo> getTenantAllDevices(
            @RequestParam int limit,
        ...
        try {
            TenantId tenantId = getCurrentUser().getTenantId();
            TextPageLink pageLink = createNaviPageLink(limit, pageIndex);
//            if (type != null && type.trim().length() > 0) {
//                return checkNotNull(deviceService.findDevicesByTenantIdAndType(tenantId, type, pageLink));
//            } else {
            return checkNotNull(deviceService.findAllDevices(tenantId, pageLink));
//            }
        } catch (Exception e) {
            throw handleException(e);
        }
    ...
    @PreAuthorize("hasAuthority('TENANT_ADMIN')")
    @RequestMapping(value = "/tenant/searchdevices", method = RequestMethod.POST)
    @ResponseBody
    public TextPageData<DeviceInfo> searchAllDevices(@RequestBody DeviceInfo deviceInfo) throws ThingsboardException {
        try {
            TenantId tenantId = getCurrentUser().getTenantId();
//            TypeId typeId = null;
//            AssetId assetId = null;
            UUID typeid, assetid;
            if(deviceInfo.getType_id() != null && !"".equals(deviceInfo.getType_id())) {
                TypeId typeId = new TypeId(toUUID(deviceInfo.getType_id()));
                typeid = typeId.getId();
            }
            else{
//                typeId = new TypeId(ModelConstants.NULL_UUID);
                typeid = null;
            }
            if(deviceInfo.getEquipid() != null && !"".equals(deviceInfo.getEquipid())) {
                AssetId assetId = new AssetId(toUUID(deviceInfo.getEquipid()));
                assetid = assetId.getId();
            }
            else{
//                assetId = new AssetId(ModelConstants.NULL_UUID);
                assetid =null;
            }

//            String typeid= deviceInfo.getType_id();
//            String equipid= deviceInfo.getEquipid();
            int pageIndex=Integer.parseInt(deviceInfo.getPageIndex());
            int limit=Integer.parseInt(deviceInfo.getLimit());
            TextPageLink pageLink = createNaviPageLink(limit, pageIndex);
//            if (type != null && type.trim().length() > 0) {
                return checkNotNull(deviceService.findAllDevices(tenantId, typeid, assetid, pageLink));
//            } else {
//            return checkNotNull(deviceService.findAllDevices(tenantId, pageLink));
//            }
        } catch (Exception e) {
...
//    @PreAuthorize("hasAuthority('TENANT_ADMIN')")
//    @RequestMapping(value = "/tenant/alldevices", params = {"limit"}, method = RequestMethod.GET)
//    @ResponseBody
//    public TextPageData<DeviceInfo> getTenantAllDevices(
//            @RequestParam int limit,
//            @RequestParam(required = false) String type,
...
//    @RequestMapping(value = "/device/types", method = RequestMethod.GET)
//    @ResponseBody
//    public List<EntitySubtype> getTenantDeviceTypes() throws ThingsboardException {
//        try {
//            SecurityUser user = getCurrentUser();
//            TenantId tenantId = user.getTenantId();
//            ListenableFuture<List<EntitySubtype>> deviceTypes = deviceService.findTenantDeviceTypes(tenantId);
//            return checkNotNull(deviceTypes.get());
//        } catch (Exception e) {
//            throw handleException(e);
//        }
//    }
    ...
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/device/types2", method = RequestMethod.GET)
    @ResponseBody
    public List<TypeSpec> getDeviceTypes() throws ThingsboardException {
        try {
            SecurityUser user = getCurrentUser();
            TenantId tenantId = user.getTenantId();
            ListenableFuture<List<TypeSpec>> deviceTypes = typeService.findDeviceTypes();
            return checkNotNull(deviceTypes.get());
        } catch (Exception e) {
            throw handleException(e);
        }
