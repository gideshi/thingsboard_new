...
/*@ngInject*/
function DeviceService($http, $q, $window,$log, userService, attributeService, customerService, types) {
    ...
    var service = {
        ...
        getTenantDevicesByQuery: getTenantDevicesByQuery,
    ...
    function getTenantDevices(pageLink, applyCustomersInfo, config, type) {
        ...
        if (angular.isDefined(pageLink.pageIndex)) {
            url += '&pageIndex=' + pageLink.pageIndex;
        }
    ...
    function getTenantDevicesByQuery( query, applyCustomersInfo, config) {
        var deferred = $q.defer();
        let deviceInfo = {};
        var url = '/api/tenant/searchdevices';
        deviceInfo.limit = query.limit;
        if (angular.isDefined(query.pageIndex)) {
            deviceInfo.pageIndex = query.pageIndex;
        }
        if (angular.isDefined(query.type_id) && query.type_id) {
            deviceInfo.type_id = query.type_id;
        }
        // else {
        //     deviceInfo.type_id = '0-0-000000';
        // }
        if (angular.isDefined(query.equipid) && query.equipid) {
            deviceInfo.equipid = query.equipid;
        }
        // else {
        //     deviceInfo.equipid = '00000000';
        // }
        $http.post(url, deviceInfo, config).then(function success(response) {
            //response.data.data = data;
            deferred.resolve(response.data);
        }, function fail() {
            deferred.reject();
    ...
    function saveDevice(device, config) {
        $log.info('service directive add');
        config = config || {};
        var deferred = $q.defer();
        var url = '/api/device';
        $log.info(device.typeId);
        var deveiceId = device.typeId;
        device.typeId = {'id': deveiceId, 'entityType':'TYPES'};
        device.customerId = null;
        // device.typeId = {'id': 'ea0f1750-dac9-11e9-aa93-bbd6f4a91b88', 'entityType':'TYPES'};
    ...
    function getDeviceTypes(config) {
        var deferred = $q.defer();
        var url = '/api/device/types2';
        $http.get(url, config).then(function success(response) {
            $log.info('service getDeviceTypes success');
            deferred.resolve(response.data);
        }, function fail() {
            deferred.reject();
        });
