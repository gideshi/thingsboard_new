...
/*@ngInject*/
function DashboardController($scope, $rootScope, $element, $timeout, $mdMedia, $mdUtil, $q, timeService, types, utils, $log) {
    ...
    $scope.$watchCollection('vm.widgets', function () {
        ...
        $log.info("--- $scope.$watchCollection--vm.widgets--");
        sortWidgets();
    ...
    $scope.$watch('vm.widgetLayouts', function () {
        updateMobileOpts();
        $log.info("--- $scope.$watchCollection-vm.widgetLayouts---");
        sortWidgets();
    });
    ...
    $scope.$on('widgetPositionChanged', function () {
        $log.info("--- $scope.$watchCollection-widgetPositionChanged---");
        sortWidgets();
    });
    $log.info("loadDashboard-2");
    loadDashboard();

    function sortWidgets() {
        $log.info('components/dashboard.directive.js sortWidgets');
    ...
    function reload() {
        $log.info('components/dashboard.directive.js reload');
        loadDashboard();
    }

    function loadDashboard() {
        $log.info('components/dashboard.directive.js loadDashboard');
        $timeout(function () {
            if (vm.loadWidgets) {
    ...
    function dashboardLoaded() {$log.info("--dashboardLoaded--");
        $mdUtil.nextTick(function () {
