...
import infoDeviceTemplate from './info-device.tpl.html';
...
export function DeviceController($rootScope, userService, deviceService, customerService, $state, $stateParams,
                                 $document, $mdDialog, $q, $translate, $log, types) {
    ...
    vm.deviceTableConfig = {

        saveItemFunc: saveDevice,

        editItemFunc: editDevice,

        deleteItemFunc: deleteDevice,

        addItemActions: deviceAddItemActionsList,

        onTableInited: tableInited,

        addItemTemplateUrl: addDeviceTemplate,

        editItemTemplateUrl: infoDeviceTemplate,
    ...
    //     vm.deviceTableConfig.items = $stateParams.items;
    // }
    ...
    function initController() {
        ...
        $log.info('vm.devicesScope:' + vm.devicesScope);
        if (vm.devicesScope === 'tenant') {
            ...
            vm.deviceTableConfig.addItemActions = [];
    ...
    function editDevice(deviceId, $event) {
        $log.info('controller editDevice:' + deviceId + $event);
        let device = deviceService.getDevice(deviceId);
        $log.info('device:' + device);
    }
    function deleteDevice(deviceId, $event) {
        $log.info('controller deleteDevice:' + deviceId + $event);
        return deviceService.deleteDevice(deviceId);
    }
    function saveDevice(device) {
        $log.info('controller saveDevice:');
        $log.info('controller saveDevice device typeId:' + device.typeId + device.toString());
