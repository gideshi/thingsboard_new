...
//    @Override
//    public List<DeviceDetailsEntity> findAllDevices(UUID tenantId,TextPageLink pageLink) {
//        return deviceRepository.findAllDevices(fromTimeUUID(tenantId),
//                        Objects.toString(pageLink.getTextSearch(), ""),
//                        pageLink.getIdOffset() == null ? NULL_UUID_STR : fromTimeUUID(pageLink.getIdOffset()),
//                        new PageRequest(0, pageLink.getLimit()));
//    }
    @Override
    public List<DeviceInfo> findAllDevices(UUID tenantId, UUID typeId, UUID assetId, TextPageLink pageLink) {
        return DaoUtil.convertInfoList(
                deviceRepository.findAllDevices(
                        fromTimeUUID(tenantId),
                        typeId ==null ?  "": fromTimeUUID(typeId),
                        assetId == null ?  "":fromTimeUUID(assetId),
//                        "",
//                        "",
                        PageRequest.of(pageLink.getPageIndex()-1,pageLink.getLimit())));
    }

    @Override
    public long findAllDevicesCount(UUID tenantId,UUID typeId,UUID assetId) {
        return deviceRepository.findAllDevicesCount(fromTimeUUID(tenantId),
                typeId ==null ?  "": fromTimeUUID(typeId),
                assetId == null ?  "":fromTimeUUID(assetId));
    }

    @Override
    public List<DeviceInfo> findAllDevices(UUID tenantId,  TextPageLink pageLink) {
        return DaoUtil.convertInfoList(
                deviceRepository.findAllDevices(
                        fromTimeUUID(tenantId),
                        "",
    ...
    @Override
    public long findAllDevicesCount(UUID tenantId) {
        return deviceRepository.findAllDevicesCount(fromTimeUUID(tenantId),"","");
    }
