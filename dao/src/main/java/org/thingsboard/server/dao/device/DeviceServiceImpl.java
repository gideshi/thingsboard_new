...
//    @Override
//    public TextPageData<DeviceInfo> findDevicesByTenantId(TenantId tenantId, TextPageLink pageLink) {
//        log.trace("Executing findDevicesByTenantId, tenantId [{}], pageLink [{}]", tenantId, pageLink);
//        validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
//        validatePageLink(pageLink, INCORRECT_PAGE_LINK + pageLink);
//        List<DeviceInfo> allDevices = deviceDao.findAllDevices(tenantId.getId(),pageLink);
//        return new TextPageData<>(allDevices, pageLink);
//    }

    @Override
    public TextPageData<DeviceInfo> findAllDevices(TenantId tenantId, UUID typeid, UUID equipid, TextPageLink pageLink) {
        log.trace("Executing findDevicesByTenantId, tenantId [{}], pageLink [{}]", tenantId, pageLink);
        validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
        validatePageLink(pageLink, INCORRECT_PAGE_LINK + pageLink);
        List<DeviceInfo> allDevices = deviceDao.findAllDevices(tenantId.getId(), typeid, equipid,pageLink);

        long datacount= deviceDao.findAllDevicesCount(tenantId.getId(), typeid, equipid);
        return new TextPageData<>(allDevices, pageLink,datacount);
    }
    @Override
    public TextPageData<DeviceInfo> findAllDevices(TenantId tenantId,  TextPageLink pageLink) {
        log.trace("Executing findDevicesByTenantId, tenantId [{}], pageLink [{}]", tenantId, pageLink);
        validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
        validatePageLink(pageLink, INCORRECT_PAGE_LINK + pageLink);
        List<DeviceInfo> allDevices = deviceDao.findAllDevices(tenantId.getId(), pageLink);

        long datacount= deviceDao.findAllDevicesCount(tenantId.getId());
        return new TextPageData<>(allDevices, pageLink,datacount);
    }
    @Override
    ...
    public ListenableFuture<List<Device>> findDevicesByQuery(TenantId tenantId, DeviceSearchQuery query) {
        ...
        devices = Futures.transform(devices, new Function<List<Device>, List<Device>>() {
            ...
            public List<Device> apply(@Nullable List<Device> deviceList) {
                return deviceList == null ? Collections.emptyList() : deviceList.stream().filter(device -> query.getDeviceTypes().contains(device.getTypeId())).collect(Collectors.toList());
            }
        });
...
//    @Override
//    public ListenableFuture<List<EntitySubtype>> findTenantDeviceTypes(TenantId tenantId) {
//        log.trace("Executing findDeviceTypesByTenantId, tenantId [{}]", tenantId);
//        validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
//        ListenableFuture<List<EntitySubtype>> tenantDeviceTypes = deviceDao.findAllTenantDeviceTypesAsync(tenantId.getId());
//        return Futures.transform(tenantDeviceTypes,
//                deviceTypes -> {
//                    deviceTypes.sort(Comparator.comparing(EntitySubtype::getTypeId));
//                    return deviceTypes;
