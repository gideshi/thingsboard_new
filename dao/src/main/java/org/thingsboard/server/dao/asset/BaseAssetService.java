...
public class BaseAssetService extends AbstractEntityService implements AssetService {
    ...
    @Override
    public List<Asset> findEquipByTenantId(TenantId tenantId) {
        log.trace("Executing findAssetsByTenantId, tenantId [{}]", tenantId);
        validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
//        validatePageLink(pageLink, INCORRECT_PAGE_LINK + pageLink);
        List<Asset> assets = assetDao.findEquipByTenantId(tenantId.getId());
        return assets;
    }
    @Override
    ...
    public ListenableFuture<List<Asset>> findAssetsByQuery(TenantId tenantId, AssetSearchQuery query) {
        ...
        assets = Futures.transform(assets, assetList ->
            assetList == null ? Collections.emptyList() : assetList.stream().filter(asset -> query.getAssetTypes().contains(asset.getTypeId())).collect(Collectors.toList())
        );
        return assets;
    }
    ...
    public ListenableFuture<List<EntitySubtype>> findAssetTypesByTenantId(TenantId tenantId) {
        ...
        return Futures.transform(tenantAssetTypes,
                assetTypes -> {
                    assetTypes.sort(Comparator.comparing(EntitySubtype::getTypeId));
                    return assetTypes;
                });
    }
    ...
    private DataValidator<Asset> assetValidator =
            new DataValidator<Asset>() {
                ...
                protected void validateDataImpl(TenantId tenantId, Asset asset) {
                    if (StringUtils.isEmpty(asset.getTypeId())) {
