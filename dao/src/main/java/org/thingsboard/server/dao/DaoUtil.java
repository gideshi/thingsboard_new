...
public abstract class DaoUtil {
    ...
    public static <T> List<DeviceInfo> convertInfoList(Collection<? extends DeviceDetailsEntity> toDataList) {
        List<DeviceInfo> list = Collections.emptyList();
        if (toDataList != null && !toDataList.isEmpty()) {
            list = new ArrayList<>();
            for (DeviceDetailsEntity object : toDataList) {
                if (object != null) {
                    list.add(object.conData());
                }
            }
        }
        return list;
